...
class MailManager
{
    ...
    public static function sendRatingMail($movieId, $score)
    {
        ...
        $message =
            ...
            ' <head></head>' .
            ' <body>' .
            '  <p>L\'utilisateur suivant : ' . SessionManager::getLoggedUser()->Nickname . ' a noté le film suivant : ' . $movie->Title . ' le ' . date('d/m/Y') . ' à ' . date('H:i') . '. Voici la note : ' . $score . '/10.</p></br><a href="localhost/movie.php?movieId=' . $movieId . '">Cliquez-ici pour voir la référence du film.</a>';
        ' </body>' .
            '</html>';
